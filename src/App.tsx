...
function App() {
  ...
  return (
    <div className="h-screen bg-gray-100 flex flex-col overflow-hidden">
      ...
      <header className="bg-white shadow-sm p-2 md:p-4 flex justify-between items-center z-10 relative">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 md:gap-2 px-2 md:px-3 py-1 rounded-full text-[10px] md:text-sm font-medium transition-colors ${
              isVADMode ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-500 border border-gray-200'
            }`}
          >
            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isVADMode ? 'bg-green-500 animate-pulse' : 'bg-gray-400'}`} />
      ...
      {/* Status Bar */}
      {isVADMode && (
        <div className="bg-blue-600 text-white text-[10px] md:text-xs font-bold py-0.5 md:py-1 px-4 text-center uppercase tracking-widest animate-in fade-in slide-in-from-top-1 duration-300">
          Status: {vadStatus}
        </div>
      )}
      ...
      <footer className="p-3 md:p-6 bg-white border-t flex justify-center z-10 relative">
        {!isVADMode ? (
          <button 
            ...
            } ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''} text-white p-5 md:p-8 rounded-full shadow-lg hover:opacity-90 transition-all transform active:scale-95`}
          >
          ...
          <div className="flex flex-col items-center gap-1 md:gap-2">
            <div className={`px-4 py-1.5 md:p-4 rounded-xl font-bold uppercase tracking-widest text-[10px] md:text-sm ${
            ...
            <button
              onClick={toggleVADMode}
              className="text-gray-500 text-[9px] md:text-xs underline"
            >
              Stop Auto-Listening
            </button>
          </div>
